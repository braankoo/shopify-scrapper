...
class ProductController extends Controller {
    ...
    public function index(Request $request): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        ...
        return DB::table('products')
            ->selectRaw('SUBSTRING_INDEX(sites.product_json,"/",3) as site,catalogs.title as catalog, products.title as product,image,CONCAT(CONCAT(CONCAT(SUBSTRING_INDEX(sites.product_json,"/",3), "/collections/"),catalogs.handle),CONCAT("/products/",products.handle)) as url, type,DATE_FORMAT(products.created_at, "%Y-%m-%d") as created_at,DATE_FORMAT(products.published_at, "%Y-%m-%d") as published_at,IFNULL(products.position,"n/a") as products.position,IFNULL(inv.quantity,"n/a") as quantity,IFNULL(inv.sales,"n/a") as sales')
            ...
            ->leftJoin(
                DB::raw("(SELECT product_id, catalog_id, sum(inventory_quantity) as quantity,sum(sales) as sales from historicals WHERE date_created = CURDATE() GROUP BY product_id,catalog_id) inv"),
                function ($join) {
